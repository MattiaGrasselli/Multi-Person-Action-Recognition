...
# Model definition
model=VGG16(input_channels=10,num_classes=6)
motion_weight_adapt(model)

#I freeze the model until the last 4 conv and final fully connected
for param in model.features[2:10].parameters():
...
# NOTE: I have removed the mean and std deviation from ImageNet  
...
training_dataset=TemporalDataset(training_csv,training_root,transform=transforms)
validation_dataset=TemporalDataset(validation_csv,validation_root,transform=transforms)
...
if __name__=='__main__':
    ...
    for epoch in range(0, N_EPOCH):
        ...
        for bathc_id, (input, label) in enumerate(training_loader):
            #print(f"Sono l\'input: {input.shape} con epoca {epoch}")
        ...
        metrics_to_save.to_csv(f'{os.getcwd()}/train_validation_metrics.csv',sep=";")
        ...
        },os.path.join('.',"last_temporal_checkpoint.pth.tar"))            
        
        if mean_validation_loss <  min_loss:
            torch.save(model.state_dict(),os.path.join('.',"temporal_best.pt"))
            ...
            torch.save({
                ...
                'optimizer_state_dict': opt.state_dict(),
                'loss': mean_validation_loss
            },os.path.join('.',"temporal_checkpoint.pth.tar"))            
