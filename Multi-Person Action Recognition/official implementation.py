...
#    print("CUDA needs to be available in order to use this software.")
...
if FPS > 30:
    ...
    out=cv2.VideoWriter(f"{VIDEO_NAME}_tracking.mp4",cv2.VideoWriter_fourcc(*'mp4v'),FPS,(int(WIDTH),int(HEIGHT)))
                ...
                if frame_count==output_twostream:
                    ...
                    for num_batch in range(batches):
                        for t in range(T):
                            ...
                            frame_tensor[num_batch,:,t,:,:]=frame_transformation(frame_list[t][int(y_1):int(y_2),int(x_1):int(x_2)])
